# Peaks DataFrame
Peaks DataFrame is a personal academic project that aims to provide an alternative to SQL statements for processing billions of rows using Go streaming model. The project started coding on February 18th, 2023 in Hong Kong SAR with the goal of achieving real-time processing up to 10 million rows per second. One of the project’s aims is to solve the data explosion that came with data capture from IoT devices. By using setting of Peaks’s script, it can support streaming and in-memory models.
