...
:: SQL Statement
duckdb -c "copy (SELECT Ledger, Account, DC, Currency, SUM(Base_Amount) as Total_Base_Amount FROM read_csv_auto('input/300-MillionRows.csv') GROUP BY Ledger, Account, DC, Currency) to 'output/DuckGroupBy.csv' (format csv, header true);"
