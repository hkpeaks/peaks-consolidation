...
func FileFolder(table_partition_store map[string]map[int]Cache, task Task, rule Rule, ir InternalRule) *map[int]Cache {
	...
	ParallelStreamingFolder := func(table_partition_store map[string]map[int]Cache, task Task, rule Rule, ir InternalRule, current_folder string) *map[int]Cache {

		CurrentExtraction2 := func(batch_size int, file_list []string, current_batch_stream map[int]map[int]Cache, task Task, rule Rule, current_folder string) {
...
func AutoStreaming(table_partition_store map[string]map[int]Cache, task Task, rule Rule, ir InternalRule) *map[int]Cache {
	...
	if total_byte < int64(rule.streamMB*1000000) { // Read file run in in-memory model
		...
		ParallelStreaming := func(table_partition_store map[string]map[int]Cache, task Task, rule Rule, ir InternalRule, total_byte int64, file *os.File) *map[int]Cache {

			CurrentExtraction := func(extraction_batch map[int]int, query_batch map[int]int, bytestream_partition map[int][]byte, streaming_count int, rule Rule, file *os.File, partition_address map[int]int64) {
			...
			CellAddress2 := func(ds DataStructure, is_column_name_exist bool, current_partition int, rule Rule, source string, bytestream_partition map[int][]byte) *Cache {
