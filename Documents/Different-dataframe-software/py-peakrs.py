...
query2 = query(    
    ...
    "group_by", "Shop, Product, Style => Count() Sum(Quantity) Sum(Amount)"
)

source_file = os.path.join("Inbox/", sys.argv[1])
result_file = [f"Outbox/PyPeakrs-Detail-Result-{os.path.basename(source_file)}", 
               f"Outbox/PyPeakrs-Summary-Result-{os.path.basename(source_file)}"]
